...
static common_sampler *new_sampler(float temp) {
    ...
    // Qwen3 recommended non-thinking settings
    sparams.temp = temp;
    sparams.top_p = 0.8f;
    sparams.top_k = 20;
    sparams.min_p = 0.0f;
...
static std::string chat_add_and_format(
    ...
    // Preserve newline behavior used by llama.cpp.
    std::ostringstream ss;

    const bool add_generation_prompt = (role == ROLE_USER);

    if (add_generation_prompt &&
        !formatted_previous.empty() &&
        formatted_previous.back() == '\n') {
        ss << "\n";
    }
    ...
    // User message needs assistant generation prompt.
    inputs.add_generation_prompt = add_generation_prompt;

    const auto formatted_new =
            common_chat_templates_apply(
                    g_chat_templates.get(),
                    inputs).prompt;

    // Extract only newly added part.
    if (formatted_new.size() < formatted_previous.size()) {
        LOGe(
            "%s: Chat template produced shorter output than previous "
            "conversation. previous=%zu new=%zu",
            __func__,
            formatted_previous.size(),
            formatted_new.size());

        throw std::runtime_error(
            "Failed to apply Qwen3 chat template");
    }

    ss << formatted_new.substr(
            formatted_previous.size(),
            formatted_new.size() - formatted_previous.size());
...
Java_com_arm_aichat_internal_InferenceEngineImpl_processUserPrompt(
    ...
    // Reset sampling state for this new assistant response.
    if (g_sampler) {
    ...
    // Feed prompt context tokens into the sampler for repetition penalty context
    for (const llama_token token : user_tokens) {
